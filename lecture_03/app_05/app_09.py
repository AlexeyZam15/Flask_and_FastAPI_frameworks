...
from lecture_03.app_10.models_05 import db, User, Post
...
def fill_tables():
    ...
    db.session.commit()
    # Добавляем статьи
    for post in range(count ** 2):
        post_n = post + 1
        author = User.query.filter_by(username=f'user{post % count + 1}').first()
        new_post = Post(title=f'Post title {post_n}', content=f'Post content {post_n}', author=author)
